...
const AddCityForm = ({ addCity }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (cityName.trim() === "") return; // Don't add empty city names
    try {
      const response = await fetch('http://localhost:8080/city/add', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setAlertMessage("Failed to add city");
      }
    } catch (error) {
      console.error('Error adding city:', error);
      setAlertMessage("Error adding city");
    }
...
const AddAreaForm = ({ cities, addArea }) => {
  ...
  const handleArea = async (e) => {
    e.preventDefault();
    if (!selectedCity || !areaName.trim()) return;
    try {
      const response = await fetch("http://localhost:8080/area/add", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ city: selectedCity, areaName }),
      });
      if (response.ok) {
